...
function insertBefore(obj,beforeobj){
  var father=beforeobj.parentNode;
  return father.insertBefore(obj,beforeobj) 
}
...
function insertAfter(obj,afterobj){
    var father=afterobj.parentNode;
  var next=getDown(afterobj,"yes");
  if (!next) {
...
function setCookie(attr,value,time){
	//键attr 值value 时间 time
	if(time==undefined){
		document.cookie=attr+"="+value;
	}else{
		var now=new Date();
		now.setTime(now.getTime()+time*10);
